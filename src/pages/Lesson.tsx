...
function getStepLabel(
  ...
  if (phase === 'reading') return `Lesson ${currentLessonIdx + 1} of ${lessonCount}`;
  if (phase === 'quiz') return `Quiz Question ${quizQuestionIdx + 1} of ${quizQuestionCount}`;
  if (isReviewMode) return 'Review Complete';
  return 'Module Complete';
}

function getProgressColor(phase: Phase, isReviewMode: boolean): string {
  if (isReviewMode) return '#22c55e';
  if (phase === 'quiz' || phase === 'quiz-result') return '#6366f1';
  return 'var(--accent)';
}

function getLessonButtonBackground(idx: number, currentLessonIdx: number): string {
  if (idx === currentLessonIdx) return 'var(--accent)';
  if (idx < currentLessonIdx) return 'var(--surface-hover)';
  return 'var(--surface)';
}

function getLessonButtonColor(idx: number, currentLessonIdx: number): string {
  if (idx === currentLessonIdx) return 'white';
  if (idx < currentLessonIdx) return 'var(--text-muted)';
  return 'var(--text-muted-strong)';
}

function getQuizResultTitle(passed: boolean, isReviewMode: boolean): string {
...
export default function Lesson() {
  ...
  const shuffleQuizRows = useCallback(
    (rows: QuizRow[]) =>
      rows.map((row) => {
  ...
  async function handleModuleComplete(finalScore: number) {
  ...
  if (phase === 'reading') {
    ...
    currentStep = moduleLessons.length + quizQuestionIdx;
  }
  ...
  const progressColor = getProgressColor(phase, isReviewMode);
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '1.5rem', marginBottom: '2rem' }}>
        <div style={{ flexGrow: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
            <span style={{ color: 'var(--text-muted)' }}>{stepLabel}</span>
          ...
          {moduleLessons.map((lesson, idx) => (
            <button
              ...
              style={{
                ...
                borderRadius: '99px',
                border: 'none',
                cursor: idx <= currentLessonIdx || isReviewMode ? 'pointer' : 'default',
                whiteSpace: 'nowrap',
                fontSize: '0.75rem',
                fontWeight: idx === currentLessonIdx ? 'bold' : 'normal',
                background: getLessonButtonBackground(idx, currentLessonIdx),
                color: getLessonButtonColor(idx, currentLessonIdx),
              ...
              >
                <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                  ...
                  <strong style={{ color: 'var(--text-primary)' }}>
                    {moduleLessons[currentLessonIdx + 1].title}
                  </strong>
                </span>
              </div>
            )}
            {currentLessonIdx === moduleLessons.length - 1 && (
              ...
              >
                <span style={{ color: '#a5b4fc', fontSize: '0.85rem' }}>
                  That&apos;s the last lesson! Next up: <strong>Module Quiz</strong> — you need 80%
                  to pass.
                </span>
              </div>
            )}
        ...
        {phase === 'quiz' && quizRows.length > 0 && (
          <>
            ...
            <p style={{ marginBottom: '1.5rem', fontSize: '1.1rem' }}>
              {quizRows[quizQuestionIdx].question}
            </p>
          ...
          (() => {
            ...
            return (
              <div style={{ textAlign: 'center' }}>
                ...
                <h2>{getQuizResultTitle(passed, isReviewMode)}</h2>
                <div
                  style={{
                    background: 'var(--surface-hover)',
                ...
                >
                  ...
                  <p style={{ fontSize: '2.5rem', fontWeight: 'bold', margin: '0 0 0.5rem' }}>
                    {quizScore} / {allQuestions.length}
                  </p>
                  <p
                    style={{
                      fontSize: '1.5rem',
                ...
                {passed ? (
                  ...
                  >
                    <p style={{ margin: '0 0 0.5rem' }}>
                      {isReviewMode ? 'Great Review' : 'XP Earned'}
                    </p>
                    {!isReviewMode && (
                ...
                <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                  ...
                  {!passed && (
                    <button type="button" className="modules__card-btn" onClick={handleRetryQuiz}>
                      Retry Quiz
                    </button>
                  )}
                  {!passed && (
