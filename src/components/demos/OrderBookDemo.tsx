...
export default function OrderBookDemo() {
  const [midPrice, setMidPrice] = useState(185.5);
  const [spread, setSpread] = useState(0.1);
  const [book, setBook] = useState(() => generateOrderBook(185.5, 0.1));
  ...
  const [trades, setTrades] = useState<
    { time: string; price: number; size: number; side: string }[]
  >([]);
  ...
  const updateBook = useCallback(() => {
    ...
    const newSpread = Math.max(0.02, Math.min(0.3, spread + (Math.random() - 0.5) * 0.02));
    setMidPrice(newMid);
  ...
  return (
    ...
    >
        ...
        <div>
          ...
          {book.bids.map((level) => (
            ...
            >
              ...
              />
              <span style={{ color: '#22c55e', fontFamily: 'monospace', zIndex: 1 }}>
                ${level.price.toFixed(2)}
              </span>
              <span style={{ color: '#888', fontFamily: 'monospace', zIndex: 1 }}>
                {level.size.toFixed(0)}
              </span>
              <span style={{ color: '#555', fontFamily: 'monospace', zIndex: 1 }}>
        ...
        <div>
          ...
          {book.asks.map((level) => (
            ...
            >
              ...
              />
              <span style={{ color: '#ef4444', fontFamily: 'monospace', zIndex: 1 }}>
                ${level.price.toFixed(2)}
              </span>
              <span style={{ color: '#888', fontFamily: 'monospace', zIndex: 1 }}>
                {level.size.toFixed(0)}
      ...
      >
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem' }}>
          <div style={{ display: 'flex', gap: '0.25rem', flex: 1 }}>
            {(['buy', 'sell'] as const).map((side) => {
              const selectedSide = orderSide === side;
              const sideBackground = side === 'buy' ? '#22c55e' : '#ef4444';
              const selectedTextColor = side === 'buy' ? 'black' : 'white';
              return (
                <button
                  key={side}
                  type="button"
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    background: selectedSide ? sideBackground : '#1a1a2e',
                    color: selectedSide ? selectedTextColor : '#888',
                    fontWeight: 'bold',
                    fontSize: '0.8rem',
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
          ...
          {orderType === 'limit' && (
            <div style={{ flex: 1 }}>
              <label
                htmlFor="order-book-limit-price"
                style={{
                  color: '#666',
                  fontSize: '0.7rem',
                  display: 'block',
        ...
        >
          Place {orderType === 'market' ? 'Market' : 'Limit'} {orderSide === 'buy' ? 'Buy' : 'Sell'}{' '}
          Order
        </button>
      </div>
      ...
      {lastTrade && (
        ...
        >
          <span
            style={{ color: lastTrade.side === 'buy' ? '#22c55e' : '#ef4444', fontWeight: 'bold' }}
          >
            {lastTrade.type === 'market' ? 'Market' : 'Limit'} {lastTrade.side.toUpperCase()}{' '}
            executed:
          </span>
      ...
      {trades.length > 0 && (
        <div style={{ background: '#111', borderRadius: '0.5rem', padding: '0.5rem' }}>
          ...
          {trades.map((t, i) => (
            <div
              key={`${t.time}-${t.side}-${t.price}-${t.size}`}
              style={{
                display: 'flex',
                justifyContent: 'space-between',
      ...
      <p style={{ color: '#666', fontSize: '0.75rem', marginTop: '0.75rem', textAlign: 'center' }}>
        Green (left) = buy orders waiting. Red (right) = sell orders waiting. The gap is the
        &quot;spread.&quot; Try placing market and limit orders to see how they interact with the
        book.
      </p>
    </div>
  );
