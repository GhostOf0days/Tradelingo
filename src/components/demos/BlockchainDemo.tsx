...
function getBlockAccentColor(isTampered: boolean, isGenesis: boolean): string {
  if (isTampered) return '#ef4444';
  if (isGenesis) return '#eab308';
  return '#22c55e';
}

function getBlockShadowColor(isTampered: boolean, isGenesis: boolean): string {
  if (isTampered) return 'rgba(239,68,68,0.15)';
  if (isGenesis) return 'rgba(234,179,8,0.1)';
    ...
    setChain((prev) =>
      prev.map((block, i) => {
        if (i === index) {
          return {
            ...block,
            data: 'TAMPERED: Hacker → Hacker: 1000 BTC',
            hash: simpleHash(`tampered${Math.random()}`),
          };
        }
        if (i > index) {
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <span
            style={{
              background: tamperedBlock !== null ? '#ef444433' : '#22c55e33',
              color: tamperedBlock !== null ? '#ef4444' : '#22c55e',
      ...
      <div style={{ overflowX: 'auto', paddingBottom: '0.5rem' }}>
        <div style={{ display: 'flex', gap: '0', alignItems: 'center', minWidth: 'max-content' }}>
          {chain.map((block, idx) => {
            ...
            const blockBorderColor = getBlockAccentColor(isTampered, isGenesis);
            const blockShadowColor = getBlockShadowColor(isTampered, isGenesis);
            return (
              <div
              ...
              >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', width: '40px' }}>
                    <div
                      style={{
                        height: '2px',
                        flex: 1,
                        background: isTampered ? '#ef4444' : '#22c55e',
                      }}
                    />
                    <div
                      style={{
                        ...
                        borderLeft: `8px solid ${isTampered ? '#ef4444' : '#22c55e'}`,
                      }}
                    />
                  </div>
                )}
                <div
                  style={{
                    ...
                    padding: '1rem',
                    minWidth: '200px',
                    boxShadow: `0 0 15px ${blockShadowColor}`,
                ...
                >
                  ...
                  >
                    <div style={{ fontSize: '0.7rem', color: '#888', marginBottom: '0.125rem' }}>
                      Data
                    </div>
                    <div
                      style={{
                        fontSize: '0.8rem',
                        color: isTampered ? '#ef4444' : '#a5b4fc',
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
        <button
          type="button"
          onClick={addBlock}
          disabled={isMining || chain.length >= 6}
          style={{
            flex: 1,
            padding: '0.6rem',
            border: 'none',
            borderRadius: '0.5rem',
            cursor: isMining || chain.length >= 6 ? 'not-allowed' : 'pointer',
            background: '#22c55e',
            color: 'black',
            fontWeight: 'bold',
            fontSize: '0.85rem',
      ...
      <p style={{ color: '#666', fontSize: '0.75rem', marginTop: '0.75rem', textAlign: 'center' }}>
        Mine blocks to build the chain. Try tampering with a block to see how the chain breaks —
        every subsequent block becomes invalid because the hashes no longer match.
      </p>
    </div>
  );
}
