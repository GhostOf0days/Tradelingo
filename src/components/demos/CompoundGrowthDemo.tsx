...
export default function CompoundGrowthDemo() {
  ...
  const drawChart = useCallback(
    (data: { year: number; contributed: number; total: number }[]) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem', justifyContent: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ color: '#888', fontSize: '0.8rem' }}>
            Contributed: {formatMoney(totalContributed)}
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <div style={{ width: 12, height: 12, borderRadius: 2, background: '#22c55e' }} />
          <span style={{ color: '#888', fontSize: '0.8rem' }}>
            Growth: {formatMoney(totalGrowth)}
          </span>
        </div>
      </div>
      ...
      <p style={{ color: '#666', fontSize: '0.75rem', marginTop: '0.75rem', textAlign: 'center' }}>
        Drag the sliders to see how changing contributions, returns, and time affects your future
        balance. Hover over the chart to see values at each year.
      </p>
    </div>
  );
}
  ...
  return (
    <div style={{ background: '#111', borderRadius: '0.5rem', padding: '0.75rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
        ...
        <span style={{ color, fontSize: '0.85rem', fontWeight: 'bold', fontFamily: 'monospace' }}>
          {format(value)}
        </span>
      </div>
      <input
