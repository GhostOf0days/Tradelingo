...
function getRiskColor(weightedRisk: number): string {
  if (weightedRisk > 20) return '#ef4444';
  if (weightedRisk > 12) return '#eab308';
  return '#22c55e';
}
...
export default function PortfolioAllocationDemo() {
  ...
  const weightedReturn = ASSETS.reduce(
    (sum, a, i) => sum + (a.expectedReturn * allocations[i]) / 100,
    0
  );
  ...
  return (
    ...
    >
            ...
            <div key={asset.name}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                  <div
                    style={{ width: 10, height: 10, borderRadius: 2, background: asset.color }}
                  />
        ...
        >
          ...
          <div
            style={{
              fontSize: '1.1rem',
              fontWeight: 'bold',
              color: getRiskColor(weightedRisk),
              fontFamily: 'monospace',
            }}
          >
      ...
      <p style={{ color: '#666', fontSize: '0.75rem', marginTop: '0.75rem', textAlign: 'center' }}>
        Adjust sliders to build your portfolio. More stocks = higher return & risk. Try the presets
        to see how different strategies balance risk and reward.
      </p>
    </div>
  );
}
